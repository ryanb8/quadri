...
impl QuadriEnumerator {
    ...
    fn get_full_horizontal_lines(x_dim: usize, y_dim: usize) -> Vec<Vec<[usize; 2]>> {
        ...
        for jx in 0..y_dim {
            let this_v = (0..x_dim).map(|x| [x, jx]).collect::<Vec<[usize; 2]>>();
            res.push(this_v);
        }
        res
    }
    fn get_full_right_diagonals(x_dim: usize, y_dim: usize) -> Vec<Vec<[usize; 2]>> {
        ...
        // left side coords
        let mut starting_coords = (0..y_dim).map(|y| [0, y]).collect::<Vec<[usize; 2]>>();

        //top coords without origin
        starting_coords.append(&mut (1..x_dim).map(|x| [x, 0]).collect::<Vec<[usize; 2]>>());

        for c in starting_coords {
            let num_steps_available = min(x_dim - c[0], y_dim - c[1]);
            let this_v = (0..num_steps_available)
                .map(|s| [c[0] + s, c[1] + s])
                .collect::<Vec<[usize; 2]>>();
            res.push(this_v);
        }
    ...
    fn get_full_left_diagonals(x_dim: usize, y_dim: usize) -> Vec<Vec<[usize; 2]>> {
        ...
        starting_coords.append(&mut (0..x_dim - 1).map(|x| [x, 0]).collect::<Vec<[usize; 2]>>());
    ...
    fn sliding_windows_of_len<T: std::clone::Clone>(
        coord_sets: Vec<Vec<T>>,
        len: usize,
    ) -> Vec<Vec<T>> {
    ...
    pub fn get_square_corners(&self) -> Vec<Vec<[usize; 2]>> {
        ...
        for ix in 0..(self.x_dim - 1) {
            for jx in 0..(self.y_dim - 1) {
                ...
                let mut these_squares = (1..max_square_distance)
                    .map(|d| [[ix, jx], [ix + d, jx], [ix, jx + d], [ix + d, jx + d]].to_vec())
...
mod test {
    ...
    fn test_sliding_windows_of_len() {
        let coord_sets = vec![vec![[0, 0], [1, 1], [2, 2], [3, 3]]];
        let expected = vec![vec![[0, 0], [1, 1], [2, 2]], vec![[1, 1], [2, 2], [3, 3]]];
    ...
    fn test_transpose_coordinates() {
        let mut c: Vec<Vec<[usize; 2]>> =
            vec![vec![[0, 1], [1, 1], [2, 1]], vec![[0, 0], [2, 0], [1, 0]]];
        let expected: Vec<Vec<[usize; 2]>> =
            vec![vec![[1, 0], [1, 1], [1, 2]], vec![[0, 0], [0, 2], [0, 1]]];
    ...
    fn test_get_full_right_diagonals() {
        let expected = vec![vec![[0, 1]], vec![[0, 0], [1, 1]], vec![[1, 0]]];
    ...
    fn test_get_full_left_diagonals() {
        let expected = vec![vec![[0, 0]], vec![[0, 1], [1, 0]], vec![[1, 1]]];
